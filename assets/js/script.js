...
// Currency and Unit Conversion Data
var currencyRates = {
  'USD': 1,
  'EUR': 0.92,
  'GBP': 0.79,
...
function calculateResult() {
    ...
    try {
        ...
        if (isNaN(result) || !isFinite(result)) {
            ...
            // Track this calculation in steps
...
function factorPrimeCheck() {
    ...
    if (isNaN(num) || !Number.isInteger(parseFloat(currentExpression)) || num < 0) {
        alert("Please enter a valid positive whole number first!");
        return;
    }
...
function updateResult() {
    ...
    if (!isNaN(num) && isFinite(num) && currentExpression.trim() === num.toString()) {
        ...
        wordArea.style.display = 'flex';
    } else if (currentExpression !== '' && !wordResult.innerHTML.includes('Prime Check')) {
